### Functions common for all MSE-based criteria (unblocked case) 
...
extend<-function(candlt)            # candlt -- treatment matrix (with labels)
{
...
initial.full<-function(cand.full)          # cand.full - matrix with labels, primary and potential terms
{
  eps<-10^(-6)
  det<-0
  while (det<eps)
