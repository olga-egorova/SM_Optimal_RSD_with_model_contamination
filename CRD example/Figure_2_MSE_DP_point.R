...
df.efficiency <- Re(as.matrix(MSEDP.output[,c(1:9, 12:14)]))

for (k in 1: nrow(df.efficiency)) {
 df.efficiency[k, 7:12] <- ifelse(df.efficiency[k, 7:12]==0,
                                  0, crit.optim*100./df.efficiency[k, 7:12])
}
...
write.csv(df.efficiency[,-1], "output/tables/MSEDP_eff.csv")
print(xtable::xtable(df.efficiency), file = "output/tables/MSEDP_eff.tex")
...
df_eff <- read.csv("output/tables/MSEDP_eff.csv", header = TRUE, sep=",")[1:10, c(2:4, 7:12)]
...
## Saving the plot
ggsave(gg_all, filename ="output/figures/Figure_MSEDP_plot.jpeg", height = 13, width = 12)
