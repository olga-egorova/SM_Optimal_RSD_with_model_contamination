...
source("functions/MSEB_CP_functions.R")
...
# Designs are forced to have two center points in each block
...
CP<-2            # number of center points per block
CP_ind<-63
...
#### scaling parameter of the prior variance, squared; tau2 = 1 in the manuscript. 
tau2 <- 1.0/Q                                 
tau <- tau2^0.5
...
seeds = c(81931, 1530, 66858, 26162, 35704, 4416)
...
for (k in 1:nrow(m.kappa)){   # might choose a subset of weights
  ...
  save.image("output/workspaces/MSEB_CP_Q_search.RData")
  write.csv(df.output, "output/tables/MSEB_CP_Q_values.csv")
...
writexl::write_xlsx(design_list, "output/designs/MSEB_CP_Q_designs.xlsx")
