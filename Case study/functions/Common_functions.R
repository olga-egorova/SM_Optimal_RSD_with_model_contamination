...
### Forming the candidate set as all possible combinations of factors levels; the same as for unblocked
...
initial.b<-function(cand,Nruns=Nruns)          # cand - matrix with labels
{
  eps<-10^(-6)
  det<-0
  while (det<eps)
  {
    ...
    X<-cand[index,]
    M<-t(X[,-1])%*%Z%*%X[,-1]
    det<-round(prod(eigen(M,symmetric=TRUE,only.values=TRUE)$values),6)
  }
  list(X=X, det=det)
}
...
# 3 factors, 5 levels each. primary - quadratic model. potential -- all up to ^3
potential.matrix.b<-function(cand.b)                        
{
