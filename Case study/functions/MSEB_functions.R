...
### Minimizing Components: DP, LoF(DP), MSE(D)
    ...
    if (move == 1){
      ...
      for (i in 1:n)
      {
        if (X1[l,1]!=cand.full[i,1])         # look at labels
        {
          ...
          if (Xcomp>Ccomp)                ### if the new design is better (min here!)
          {
            X1<-Xc1; X2<-Xc2
            Xcomp<-Ccomp
            search<-1
...
{
  start_time<-Sys.time()
  cand<-candidate_set.b(Levels)                       # form the candidate set of treatments, primary terms, not(!) orthonormal
