##########################################################################################
## Main script -- obtaining GD-optimal designs, adding 5 center points and 
...
seeds = c(207173, 193821, 69132, 90674, 58843, 319778, 6838, 60460, 155363, 3001)
...
MSE_optim <- read.csv("output/MSEDP_point_values.csv", header = TRUE)[, c(7:9, 11)]
...
for (k in 1:nrow(m.kappa)){   # 
  ...
  df.output[k,] <- c(m.kappa[k,], "GD", "5CP", "1",
                     out$df, 
                     MSE_optim[k,]/c(out$DP, out$LoFDP, out$mse.point, compound))
  
  
  save.image("output/workspaces/GD_5CP.RData")
  write.csv(df.output, "output/tables/GD_5CP.csv")
