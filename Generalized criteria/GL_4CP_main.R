##########################################################################################
## Main script -- obtaining GL-optimal designs, adding 4 center points and 
...
tau2<-1.0         # scaling parameter of the prior variance, squared
tau<-sqrt(tau2)
...
seeds = c(16633, 193821, 69132, 90674, 58843, 319778, 6838, 60460, 155363, 3001)
...
MSE_optim <- read.csv("output/MSELP_values.csv", header = TRUE)[, c(7:9, 14)]
...
for (k in 1:nrow(m.kappa)){   # 
  ...
  df.output[k,] <- c(m.kappa[k,], "GL", "4CP", "1", 
                     out$df, 
                     MSE_optim[k,]/c(out$LP, out$LoFLP, out$mseL, compound))
  
  save.image("output/workspaces/GL_4CP.RData")
  write.csv(df.output, "output/tables/GL_4CP.csv")
